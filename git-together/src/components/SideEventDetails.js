...
class SideEventDetails extends React.Component {
 ...
 render() {
   const regex = /(<([^>]+)>)/ig
   const mapLink = `http://maps.google.com/?q=${this.props.selectedEvent.venue.address_1}+${this.props.selectedEvent.venue.city}`
      ...
      <button className="closebtn" onClick={() => {this.props.selectEvent(false)}}>X</button>
       ...
       <h5>Group: {this.props.selectedEvent.group.name}</h5>
        <div>
          ...
          <a href={mapLink} target="_blank"> <img src="./Maps-icon-16.png"/> {this.props.selectedEvent.venue.address_1} {this.props.selectedEvent.venue.city}</a>
        </div>
       <p>{this.props.selectedEvent.description.replace(regex, " ")}</p>
